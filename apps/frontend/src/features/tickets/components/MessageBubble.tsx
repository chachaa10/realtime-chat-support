...
function AttachmentPreview({
  ...
  useEffect(() => {
    let cancelled = false;
    fetchAttachmentAsBlob(attachmentId)
      .then(async (blob) => {
        if (cancelled) return;
  ...
  if (error) {
    return (
      <a
        href={`${API_BASE}/uploads/${attachmentId}`}
        target="_blank"
        rel="noopener noreferrer"
        className="bg-ink/5 text-ink hover:bg-ink/10 flex items-center gap-2 rounded-lg px-3 py-2 text-[0.75rem] transition-colors"
      >
        <svg
          width="13"
          height="13"
  ...
  if (!url) {
    return <div className="bg-ink/10 h-24 w-48 animate-pulse rounded-lg" />;
  }
  ...
  function renderThumbnail() {
      ...
      return (
        <>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-black/50 transition-colors group-hover:bg-black/60">
    ...
    if (pdf || text) {
      return (
        <div className="bg-ink/5 flex items-center gap-2 rounded-lg px-3 py-3 text-[0.8125rem]">
          <svg
            width="18"
            height="18"
            viewBox="0 0 24 24"
          ...
          <span className="text-ink truncate">{fileName}</span>
        </div>
      );
    }
  ...
  function renderModal() {
    if (video) {
      return <video src={url} className="max-h-[90vh] max-w-[90vw] rounded-lg" controls autoPlay />;
    }
      ...
      return (
        <div className="max-h-[85vh] w-[90vw] overflow-y-auto rounded-lg bg-white">
          <Document
            file={url}
            loading={
              <div className="text-ink-dim flex h-48 items-center justify-center text-sm">
                Loading PDF...
              </div>
            }
          >
    ...
    if (text) {
      return (
        <pre className="max-h-[80vh] max-w-[80vw] overflow-auto rounded-lg bg-[#1e1e2e] p-6 text-[0.8125rem] whitespace-pre-wrap text-[#cdd6f4]">
          {textContent ?? 'Loading...'}
        </pre>
      );
    }
      ...
      {open && (
        ...
        >
          <div className="relative max-h-[90vh] max-w-[90vw]" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              onClick={close}
              className="bg-ink/80 hover:bg-ink absolute -top-3 -right-3 flex h-8 w-8 items-center justify-center rounded-full text-white transition-colors"
              aria-label="Close"
            >
              <svg
                width="16"
    ...
    <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
      <div
        className={`max-w-[75%] rounded-xl px-4 py-2.5 ${
          isOwn ? 'bg-brand text-primary-foreground' : 'bg-surface-raised border-border border'
        }`}
      >
        {!isOwn && (
          <span className="text-ink-dim mb-1 block text-[0.6875rem] font-medium">{authorName}</span>
        )}
        <p className="text-[0.8125rem] leading-relaxed break-words whitespace-pre-wrap">
          {message.body}
        </p>
